...
public class MyAudioRecorder {
    ...
    private static MyAudioRecorder result = null;
    ...
    public static MyAudioRecorder getInstanse(Boolean recordingCompressed) {
        if (recordingCompressed) {
            result = new MyAudioRecorder(false, AudioSource.MIC,
                    sampleRates[3], AudioFormat.CHANNEL_CONFIGURATION_MONO,
                    AudioFormat.ENCODING_PCM_16BIT);
        } else {
            int i = 3;
            do {
                result = new MyAudioRecorder(true, AudioSource.MIC,
                        sampleRates[i], AudioFormat.CHANNEL_CONFIGURATION_MONO,
                        AudioFormat.ENCODING_PCM_16BIT);

            } while ((++i < sampleRates.length)
                    & !(result.getState() == MyAudioRecorder.State.INITIALIZING));
        }
        return result;
    }
    ...
    private AudioRecord.OnRecordPositionUpdateListener updateListener = new AudioRecord.OnRecordPositionUpdateListener() {
        public void onPeriodicNotification(AudioRecord recorder) {
            ...
            try {
                ...
                Log.e(MyAudioRecorder.class.getName(),
...
//                stop();
     ...
     */
    public MyAudioRecorder(boolean uncompressed, int audioSource,
        ...
        try {
            ...
            if (rUncompressed) { // RECORDING_UNCOMPRESSED
                ...
                if (bufferSize < AudioRecord.getMinBufferSize(sampleRate,
                    ...
                    Log.w(MyAudioRecorder.class.getName(),
    ...
    private int calculateDecibel() {
        if(buffer==null) return 0;
        int sum = 0;
        int bs = buffer.length;
        for (byte aBuffer : buffer) {
            sum += Math.abs(aBuffer);
        }
        // avg 10-50
        return sum / bs;
    }
